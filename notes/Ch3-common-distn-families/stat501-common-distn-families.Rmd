...
\begin{mdframed}[skipabove = 8pt, skipbelow = 8pt]
\textbf{DEF 3.5.5 (Location-Scale Family)} Let $f_X(x)$ be any pdf. Then the family of pdfs $\displaystyle \frac{1}{\sigma}f_X\left(\frac{x-\mu}{\sigma}\right)$, indexed by the parameters $\mu$ and $\sigma$, where $-\infty < \mu < \infty$ and $\sigma > 0$, is called the \textbf{location-scale family} with the standard pdf $f_X(x)$; $\mu$ is called the location parameter and $\sigma$ is called the scale parameter for the family. That is, the standard (or reference) pdf is where $\mu = 0$ and $\sigma = 1$. 
\end{mdframed}
...
\begin{mdframed}[skipabove = 8pt, skipbelow = 8pt]
\textbf{THM 3.5.6 (Generating location, scale, and location-scale families)} Let $f(\cdot)$ be any pdf. Let $\mu$ be any real number, and let $\sigma$ be any positive real number. Then, $X$ is a random variable with pdf $f_X(x) = \frac{1}{\sigma}f\left(\frac{x- \mu}{\sigma}\right)$ if and only if there exists a random variable $Z$ with pdf $f(z)$ and $X = \sigma Z + \mu$. 
\end{mdframed}
